...
for image in "$WALLPAPER_DIR"/*.{jpg,JPG,jpeg,JPEG,png,PNG,gif,GIF,webp,WEBP}; do
    ...
    if [[ "$image" =~ \.gif$|\.GIF$ ]]; then
        magick "$image[0]" -resize 960x540^ -gravity center -extent 960x540 "$THUMBNAIL_DIR/$filename"
    else
        magick "$image" -resize 960x540^ -gravity center -extent 960x540 "$THUMBNAIL_DIR/$filename"
    fi
done
