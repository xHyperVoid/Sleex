...
import {
     ...
     geminiTabIcon,
} from "../widgets/apis/gemini.js";
import {
     chatGPTView,
     chatGPTCommands,
     sendMessage as chatGPTSendMessage,
     chatGPTTabIcon,
} from "../widgets/apis/chatgpt.js";
     ...
     setup: (self) =>
          self
               ...
               .hook(
                    GPTService,
                    (self) => {
                         if (APIS[currentApiId].name != "Assistant (GPTs)")
                              return;
...
chatEntry.get_buffer().connect("changed", (buffer) => {
     ...
     if (
          ...
          chatEntryWrapper.toggleClassName(
...
const apiWidgets = () =>
     Widget.Box({
          attribute: {
               nextTab: () =>
                    switchToTab(Math.min(currentApiId + 1, APIS.length - 1)),
